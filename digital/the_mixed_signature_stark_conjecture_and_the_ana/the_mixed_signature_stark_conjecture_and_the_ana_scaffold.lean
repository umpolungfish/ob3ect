-- IGProtocol scaffold: VINIT → AFWD → FSPLIT → EVALT → EVALF → ENGAGR → CLINK → FFUSE → IMSCRIB → IFIX → TANCH
-- Class: The mixed-signature Stark conjecture and the analytic passage to the C^2 metric
