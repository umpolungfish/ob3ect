-- IGProtocol scaffold: VINIT → AFWD → FSPLIT → AFWD → EVALT → AREV → EVALF → CLINK → ENGAGR → FFUSE → IMSCRIB → IFIX → TANCH
-- Class: Optimal Tarot Spread and Procedure (Autodidactic)
