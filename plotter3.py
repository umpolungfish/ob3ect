...
# ══════════════════════════════════════════════════════════════════
#TOP-DOWN (xy plane, z=0) — EQUATORIAL CROSS-SECTION
# ══════════════════════════════════════════════════════════════════
ax2.set_title('VIEW 3: Top-Down (xy plane, z=0)', 
              color=WHITE, fontsize=14, fontweight='bold', pad=12)

ax2.plot(2*R*np.cos(theta), 2*R*np.sin(theta), color='#5a659c', lw=2.5, alpha=0.35)

ax2.plot(np.array([horn_xy(t) for t in theta])[:,0],
...
ax2.text(-0.35, -0.45, r'$\odot$ (pinch)', color=GOLD, fontsize=9, fontweight='bold', ha='right')
ax2.text(2.1, -0.35, r'$\ni$ FFUSE3', color=BLUE, fontsize=8, fontweight='bold')
ax2.text(1.15, 1.1, '+ EVALT', color=BLUE, fontsize=8, fontweight='bold')
ax2.text(1.15, -0.65, r'$\times$ EVALF', color=RED, fontsize=8, fontweight='bold')
ax2.text(horn_xy(0)[0]+0.1, horn_xy(0)[1]-0.5, 'horn(0)', color=PURPLE, fontsize=8)
...
ax2.text(0.4, 0.55, r'$\sqrt{2}$', color=GREEN, fontsize=11, fontweight='bold')
ax2.text(0.55, 0.25, r'$\sqrt{3}$ (3D)', color=GREEN, fontsize=11, fontweight='bold')
ax2.text(1.0, -0.2, '2', color=CYAN, fontsize=12, fontweight='bold', ha='center')
ax2.text(0.5, -0.2, r'$\lambda_C$', color=ORANGE, fontsize=10, fontweight='bold')

ax2.plot([0, 4], [0, 0], color=PURPLE, ls='--', lw=2, alpha=0.6)
ax2.text(2.1, 0.35, 'syzygy axis', color=PURPLE, fontsize=8, alpha=0.8)
...
ax2.text(phi_plus[0]+0.2, phi_plus[1]+0.15, r'$\phi$-tangent', color=WHITE, fontsize=7)
...
# ══════════════════════════════════════════════════════════════════
ax3.set_title('VIEW 2: Face-On Ortho (yz plane, x=0)', 
              color=WHITE, fontsize=14, fontweight='bold', pad=12)
...
ax3.text(-R, -2.4, 'Left Lobe (y=-2)', color='#5a659c', fontsize=8, ha='center')
ax3.text(R, -2.4, 'Right Lobe (y=+2)', color='#5a659c', fontsize=8, ha='center')
...
ax3.plot(r_split * np.cos(theta), r_split * np.sin(theta), color=ORANGE, ls='--', lw=2.5, zorder=10, label=r'Bloch ($\lambda_C$) Shell')
ax3.plot(r_split * np.cos(theta), r_split * np.sin(theta), color=BLUE, ls=':', lw=2.0, zorder=11, label='FFUSE3 Sphere')
ax3.fill(r_split * np.cos(theta), r_split * np.sin(theta), color=GOLD, alpha=0.12, zorder=9)

# Center pinch / vector origin bullseye
ax3.scatter(0, 0, color=GOLD, s=280, zorder=15, edgecolors=WHITE, lw=1.5)
ax3.scatter(0, 0, color=BLUE, s=100, zorder=16)
ax3.text(0, -0.4, r'$\odot$ pinch / Bloch-FFUSE3 overlap', color=WHITE, fontsize=8, fontweight='bold', ha='center')
...
ax3.text(ev_y[0]+0.2, ev_z[0], '+ EVALT', color=BLUE, fontsize=8, fontweight='bold', va='center')
ax3.text(ev_y[1]-0.2, ev_z[1]+0.2, r'$\times$ EVALF', color=RED, fontsize=8, fontweight='bold', ha='right')
ax3.text(ev_y[2]-0.2, ev_z[2]-0.2, r'$\sqcap$ EVALI', color=RED, fontsize=8, fontweight='bold', ha='right')
