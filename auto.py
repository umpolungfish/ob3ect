...
DECISION VS ITERATION: FSPLIT models a genuine either/or, two arms that could each be
taken because the domain actually forks on a choice. Building n similar items in
sequence (n cliques, n rounds of one process, n members of a family) is not a decision
and takes a CLINK chain, one CLINK per item, never an FSPLIT, even when n is a variable
and even when there are many of them. A fork earns its FFUSE by putting genuinely
different content on each arm. If nothing distinguishes what happens on the two arms, or
nothing happens on one of them, it is not a fork, it is decoration: the kernel's own
insert-repair reads such a fork as vacuous (no clear ever fired, nothing was at risk) or
reports the word already holds with nothing to add. Do not write a fork you would not
want read back that way.
...
_REPAIR_ALREADY_HOLDS = re.compile(r"^already holds — nothing to repair$")
_REPAIR_EXPOSED = re.compile(r"^exposed:\s*(\d+)\s+unit\(s\) cleared with nothing banked$")
...
async def _kernel_repairs(word: str, timeout: int = 30) -> Optional[dict]:
    ...
    already_holds = any(_REPAIR_ALREADY_HOLDS.match(l) for l in lines)
    exposed_m = next((m for m in (_REPAIR_EXPOSED.match(l) for l in lines) if m), None)
    holds = [{"glyph": m.group(1), "position": int(m.group(2)), "word": m.group(3)}
             for m in (_REPAIR_HOLD.match(l) for l in lines) if m]
    counted = [c for c in (_REPAIR_COUNT.search(l) for l in lines) if c]
    counts = ({"hold": int(counted[0].group(1)), "tried": int(counted[0].group(2))}
              if counted else None)
    vacuous = any(l.startswith("vacuous:") for l in lines)
    exposed_lost = int(exposed_m.group(1)) if exposed_m else None
    # `already_holds` and `exposed_lost` used to fall through both patterns and
    # the vacuous check, landing on this function's own None — indistinguishable
    # from "the kernel call produced nothing parseable". The two cases it hid are
    # opposite findings: already_holds means the word needs no repair at all,
    # exposed_lost means it lost units with nothing banked, a real defect.
    if not (already_holds or vacuous or exposed_lost is not None or counts or holds):
        return None
    return {
        "already_holds": already_holds,
        "vacuous": vacuous,
        "exposed_lost": exposed_lost,
        "holds": holds,
        "counts": counts,
    }
...
def _print_artifact_block(art: "Ob3ectArtifact", no_scaffold: bool = False) -> None:
    ...
    if rep.get("already_holds"):
        out.append("\nKernel repair: already holds — nothing to repair")
    elif rep.get("exposed_lost") is not None:
        out.append(f"\nKernel repair: EXPOSED — {rep['exposed_lost']} unit(s) "
                   f"cleared with nothing banked")
    elif rep.get("vacuous"):
        out.append("\nKernel repair: vacuous — no clear ever fired, nothing was at risk")
    if counts:
